...
namespace Moycroft\CLI\processors\functions\toggle\chron;
...
function run($service=null, $action=null, $processor=null, $status, $route=null){
    ...
    if ($processor != null){
        if (!$connect->query("SELECT `processor` FROM `internal_chron_processors` WHERE `route` = '$route' AND `processor` = '$processor';", true)){
            ...
            $connect->query("UPDATE `internal_chron_processors` SET `enabled`='$status' WHERE `route` = '$route' AND `processor` = '$processor';");
        }
    }
    if ($action != null && $service != null){
        if (!$connect->query("SELECT `processor` FROM `internal_chron_processors` WHERE `route` = '$route' AND `service` = '$service' AND `action` = '$action';", true)){
            ...
            $connect->query("UPDATE `internal_chron_processors` SET `enabled`='$status' WHERE `route` = '$route' AND `service` = '$service' AND `action` = '$action';");
        }
    }
}